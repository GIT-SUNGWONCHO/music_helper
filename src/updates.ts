...
export const UPDATE_LOG: UpdateEntry[] = [
  { version: '0.15.0', date: '2026-07-23', notes: ['섹션 맨 뒤 마디 추가 버튼 제거(마디별 삽입 기능으로 대체)', '설정을 일반/관리자 탭으로 분리, 코드색 버튼을 설정 > 일반으로 이동', '코드 기본색을 코랄핑크로 변경', 'AI 생성 화면에 추가 요청사항(자유 프롬프트) 입력칸 추가, 참고 악보 섹션 제목/설명 위계 및 이미지·링크 라벨 정리', '코드표에서 자주 쓰는 코드가 앞쪽에 오도록 정렬, 코드 추가 패널을 운지 다이어그램 가로 스크롤로 변경', '코드 추가 패널 루트 음 선택 줄이 화면 밖으로 넘쳐 전체 화면이 눌려 보이던 버그 수정'] },
